...
test('live card assembly pools three same-season leagues and withholds a cameo target', async () => {
  ...
  try {
    ...
    assert.ok(low.categories.every((c) => c.percentile === null && c.league_percentile === null));
    assert.ok(low.categories[0].components.every((m) => m.value === 10000 && m.percentile === null));
